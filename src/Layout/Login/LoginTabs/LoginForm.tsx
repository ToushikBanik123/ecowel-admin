...
const LoginForm = () => {
  ...
  const { data: session, status } = useSession({
    required: true,
    onUnauthenticated() {
      router.push(`/${i18LangStatus || 'en'}/auth/login`);
    },
  });
   const user = session?.user;

   if(!user) router.push(`/${i18LangStatus || 'en'}/auth/login`);


  console.log("login session====================================================================>", session, user, status);
    ...
    const login = async () => {
      try {
        ...
        if (res?.url) {
          ...
          router.push(`/${i18LangStatus || 'en'}/dashboard`);
